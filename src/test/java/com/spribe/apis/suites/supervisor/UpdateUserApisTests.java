package com.spribe.apis.suites.supervisor;
...
@Epic("Update users")
@Feature("Update different type of users by Supervisor role")
public class UpdateUserApisTests extends PlayerControllerTests {
    ...
    @Test
    @Description("Check that Supervisor is able to update editable fields(ell except role) User and check response body.")
    ...
    @Test
    @Description("Check that Supervisor is able to update editable fields(ell except role) User and check get user data.")
    ...
    @Test
    @Description("Check that Supervisor is able to update editable fields(ell except role) Admin and check response body.")
    ...
    @Test
    @Description("Check that Supervisor is able to update editable fields(ell except role) Admin and check get user data.")
    ...
    @Test(enabled = false)
    @Description("Check that Supervisor is able to update editable fields(ell except role) Supervisor/itself and check response body.")
...
//        User user = usersApiActions.userWithRoleIsCreated(PlayerControllerUserRole.SUPERVISOR);
        ...
        jsonBody.addProperty("age", newUserData.getAge());
        jsonBody.addProperty("gender", newUserData.getGender().getGenderPropertyText());
//        TODO Test is disabled until there is possibility to create new test instances for Supervisor role
//        jsonBody.addProperty("login", newUserData.getLogin());
        jsonBody.addProperty("password", newUserData.getPassword());
        jsonBody.addProperty("screenName", newUserData.getScreenName());
        ...
        Response response = spribeApi.playerControllerRequest()
                .contentType(ContentType.JSON)
                .body(jsonBody.toString())
                .patch(String.format("player/update/%s/%s", editor.getLogin(), editor.getId()));

        logger.info("Update user response: " + response.asString());

        if (response.statusCode() == 200) {
            editor.updateUserData(newUserData);
        }
        ...
        Assert.assertFalse(JsonParser.parseString(response.asString()).getAsJsonObject().has("password"),
                "Field 'password' should not be displayed.");
        Assert.assertEquals(response.jsonPath().getString("role"), editor.getRole().getRolePropertyText(),
                "Field 'role' doesn't match.");
        Assert.assertEquals(response.jsonPath().getString("screenName"), newUserData.getScreenName(),
                "Field 'screenName' doesn't match.");
    }
